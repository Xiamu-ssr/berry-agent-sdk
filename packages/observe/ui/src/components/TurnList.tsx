...
export function TurnList({ baseUrl, sessionId, agentId, limit = 50, onSelect }: Props) {
  ...
  if (!data || data.length === 0) return <div className="text-gray-400 p-4">No turns recorded</div>;
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="text-sm text-gray-700 truncate">
                {t.prompt ? `"${t.prompt.slice(0, 80)}${t.prompt.length > 80 ? '...' : ''}"` : <span className="text-gray-400 italic">no prompt</span>}
              </div>
            ...
            <div className="flex items-center gap-4 ml-4 text-xs text-gray-500 shrink-0">
              ...
              <span className="font-medium text-gray-700">${t.totalCost.toFixed(4)}</span>
              <span className={`px-2 py-0.5 rounded ${t.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                {t.status}
              </span>
